...
public class TestData implements TutorialData {

    private static final Logger baseInsert = LoggerFactory.getLogger("baseInsert");
    private static final Logger baseSelect = LoggerFactory.getLogger("baseSelect");
    ...
    public SimpleFeatureType getSimpleFeatureType() {
        if (sft == null) {
            ...
            attributes.append("lat:String,");
            attributes.append("lon:String,");
            attributes.append("province:String,");
            attributes.append("city:String,"); // marks this attribute for indexing
    ...
    public List<SimpleFeature> getTestData() {
        ...
        if (features == null) {
            ...
            URL input = null;
            try {
                input = new URL("file:///home/cklu/geomesa/input_data/test.csv");
            } catch (MalformedURLException e){
                e.printStackTrace();
            }
            baseInsert.info(input.toString());

            // date parser corresponding to the CSV format
            DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("d/M/yyyy HH:mm:ss", Locale.CHINA);
...
//                    logger.info(record.toString());
                    try {
                        ...
                        builder.set("truckNo", record.get(0));
//                        logger.info("1");
                        ...
                        try {
                            builder.set("utc",
                                    Date.from(LocalDateTime.parse(record.get(1), dateFormat).atZone(ZoneOffset.UTC).toInstant()));
                        }catch (Exception e){
                            e.printStackTrace();
                        }
//                        logger.info("2");
                        builder.set("province", record.get(4));
//                        logger.info("3");
                        builder.set("city", record.get(5));
//                        logger.info("4");
                        builder.set("country", record.get(6));
//                        logger.info("5");
...
//                        logger.info("7");
                        ...
                        if (i<5) {
                            baseInsert.info(feature.toString());
                            i++;
                        }
                    } catch (Exception e) {
                        baseInsert.debug("Invalid GDELT record: " + e.toString() + " " + record.toString());
                    }
                }
            } catch (IOException e) {
    ...
    public List<Query> getTestQueries() {
        if (queries == null) {
            try {
                ...
                String during = "utc DURING 2021-08-01T00:00:00.000Z/2021-08-01T10:00:00.000Z";
                // bounding box over most of the united states
                String bbox = "bbox(geom,117,35,120,36)";
                ...
                queries.add(new Query(getTypeName(), ECQL.toFilter(bbox + " AND " + during),
                                      new String[]{ "truckNo", "utc", "geom" }));
                // attribute query on a secondary index - note we specified index=true for truckNo
                queries.add(new Query(getTypeName(), ECQL.toFilter("truckNo = '鲁L70373'")));
                // attribute query on a secondary index with a projection
                queries.add(new Query(getTypeName(), ECQL.toFilter("truckNo = '鲁L70373' AND " + during),
                                      new String[]{ "truckNo", "utc", "geom" }));

                this.queries = Collections.unmodifiableList(queries);
                baseSelect.info(this.queries.toString());
