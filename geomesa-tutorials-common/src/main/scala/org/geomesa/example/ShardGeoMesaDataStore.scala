...
import org.geomesa.example.ShardGeoMesaDataStore._
...
class ShardGeoMesaDataStore(looseBBox: Boolean)
  extends GeoMesaDataStore[ShardGeoMesaDataStore](ShardConfig(looseBBox)) with LocalLocking {

  override val metadata: GeoMesaMetadata[String] = new InMemoryMetadata[String]

  override val adapter: ShardIndexAdapter = new ShardIndexAdapter(this)

  override val stats: GeoMesaStats = new ShardStats(this, new InMemoryMetadata[Stat]())
...
object ShardGeoMesaDataStore {

  class ShardIndexAdapter(ds: ShardGeoMesaDataStore) extends IndexAdapter[ShardGeoMesaDataStore] {
    ...
    override def createQueryPlan(strategy: QueryStrategy): QueryPlan[ShardGeoMesaDataStore] = {
  ...
  case class TestQueryPlan(
                          ...
                          ) extends QueryPlan[ShardGeoMesaDataStore] {
    ...
    override def scan(ds: ShardGeoMesaDataStore): CloseableIterator[SimpleFeature] = {
  ...
  case class ShardConfig(looseBBox: Boolean) extends GeoMesaDataStoreConfig {
    ...
    override val queries: DataStoreQueryConfig = new DataStoreQueryConfig() {
      ...
      override def looseBBox: Boolean = ShardConfig.this.looseBBox
    }
    override val namespace: Option[String] = None
  }

  class ShardStats(ds: ShardGeoMesaDataStore, metadata: GeoMesaMetadata[Stat])
